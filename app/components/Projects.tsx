...
function Tag({ label, color }: { label: string; color?: string }) {
  return (
    <span className={`inline-block px-3 py-0.5 rounded-full text-[11px] font-semibold tracking-wide text-white ${color || "bg-blue-600"}`}>
      {label}
    </span>
  );
}
...
function StatsBadges({ stats }: { stats: string[] }) {
  return (
    <div className="flex items-center gap-2 mt-2">
      {stats.map((s, i) => (
        <span key={i} className="text-[11px] font-bold text-white/60 bg-white/10 rounded px-2 py-0.5">
          {s}
        </span>
      ))}
    </div>
...
// Tech logos row
function TechLogos({ tech }: { tech: string[] }) {
  return (
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="text-center mb-10" data-aos="fade-up">
            ...
            <p className="text-white/70 text-lg max-w-md mx-auto">
              From crypto exchanges to healthcare platforms — we build digital products that scale globally.
            </p>
          </div>
            ...
            >
              <div className="flex flex-col sm:flex-row items-center">
                ...
                <div className="flex flex-col justify-center p-6 w-full sm:w-[45%] gap-3">
                  ...
                  {/* Featured tech logos */}
                  <TechLogos tech={featuredProject.tech} />
